...
# 第二引数(必須) : 2点の座標の1つ。矢印の始点。tuple型。
# 第三引数(必須) : 2点の座標のもう1つ。矢印の終点。tuple型。
# 第四引数(必須) : 矢印の色を指定する。B(Blue)G(Green)R(Red)形式で指定する。tuple型。
# thickness : 矢印の太さ(px)を指定する。
# tipLength : 矢印の先の長さを設定するために利用します。float型。
cv2.arrowedLine(img, (200, 250), (300, 300), (255, 0, 0), thickness=4, tipLength=0.5)
